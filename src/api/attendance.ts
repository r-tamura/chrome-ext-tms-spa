...
import { remap } from "~/helpers/common";
import {
...
import { createMonthlyId } from "~/modules/attendances";
import {
...
const clientToServer = (client: Partial<AttendanceDaily>) =>
...
const saveMonthlyAttendances = async (
  ...
  const attendancesOnServer = attendances
    .map(clientToServer)
  ...
  const promises = attendancesOnServer.map(async attendance => {
  ...
  return okResults.length === attendancesOnServer.length
    ...
    : {
        status: Status.NG,
        message: `${attendancesOnServer.length -
          okResults.length} attendance requests failed`
      };
};
...
const submitApplication = async (
  ...
  const html = await post(urls.ATTENDANCE_EDIT, {
    func: "commit",
    ...summary,
    eym: createMonthlyId(year, month)
  });
...
const patchSettings = async (
  ...
  Promise.resolve(getSettings())
    ...
    .then(nextSettings =>
      localStorage.setItem(LS_ATTND_SETTINGS, JSON.stringify(nextSettings))
    )
    .then(_ => ({
      status: Status.OK,
