...
export const arrayToMap = <T>(array: T[], keyName: string, valueName: string): Map<string, string> => {
  const map = new Map<string, string>()
  array.forEach((e: T) => {
    map.set(R.prop(keyName, e), R.prop(valueName, e))
  })
  return map
}

export const aryOfObjToObj = <T>(keyName: string, aryOfObj: T[]): { [s: string]: T } =>
  aryOfObj.reduce((acc, v) => ({ ...acc, [R.prop<string>(keyName, v)]: v}), {})
...
export const composeAsync = (...fns: Array<(...args: any[]) => any>): (...args: any[]) => any => {
  if (fns.length === 0) {
    return (arg: any) => arg
  }
...
export const uuidv4 = (): string =>
  Array
    .from("xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx")
    .map(c => {
      switch (c) {
      case "x":
