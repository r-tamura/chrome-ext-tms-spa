...
export const fetchExpenseTemplatesAll = () => (
  ...
  try {
    composeAsync(dispatch, fetchTransExpensesSuccess, fetchTemplatesAll)(
      master
    );
  } catch (err) {
...
export const fetchTransExpensesSuccess = (
  templates: TransExpenseTemplate[]
) => ({
  type: ActionTypes.TRANS_EXPENSES_TEMPLATE_FETCH_SUCCESS,
  templates,
...
const successOrFailToUpdate = (
  ...
  if (result.status === Status.OK) {
    ...
    return composeAsync(dispatch, updateTransExpensesFailure)(result);
  }
};
...
export const createExpenseTemplate = (template: TransExpenseTemplate) => {
  return (dispatch: Dispatch<AnyAction>, getState: () => RootState) => {
    ...
    try {
      composeAsync(successOrFailToUpdate(dispatch, getState), createTemplate)(
        template
      );
    } catch (err) {
...
export const deleteExpenseTemplate = (templateId: string) => {
  return (dispatch: Dispatch<AnyAction>, getState: () => RootState) => {
    dispatch(updateTransExpensesRequest());
    try {
      composeAsync(successOrFailToUpdate(dispatch, getState), deleteTemplate)(
        templateId
      );
    } catch (err) {
      dispatch(updateTransExpensesFailure({ error: err }));
...
export const updateExpenseTemplate = (template: TransExpenseTemplate) => (
  ...
  dispatch(updateTransExpensesRequest());
  try {
    composeAsync(successOrFailToUpdate(dispatch, getState), updateTemplate)(
      template
    );
  } catch (err) {
    dispatch(updateTransExpensesFailure({ error: err }));
...
export const updateTransExpensesFailure = ({
  result,
  error
}: {
  result?: ResultStatus;
...
export type TransExpenseAction =
  | ITransExpensesTemplateFetchRequest
  | ITransExpensesTemplateFetchSuccess
  | ITransExpensesTemplateFetchFailure
  | ITransExpensesTemplateUpdateRequest
  | ITransExpensesTemplateUpdateSuccess
  | ITransExpensesTemplateUpdateFailure;
...
export const getTransExpenseTemplates = (
  state: TransExpenseTemplateState
): TransExpenseTemplate[] => state.allIds.map(id => state.byId[id]);

export const getTransExpenseTemplate = (
  state: TransExpenseTemplateState,
  templateId: string
): TransExpenseTemplate => state.byId[templateId];

export const getIsFetching = (state: TransExpenseTemplateState) =>
  state.isFetching;
export const getIsUpdating = (state: TransExpenseTemplateState) =>
  state.isUpdating;
...
function byId(state: ById = {}, action: TransExpenseAction): ById {
  switch (action.type) {
    case ActionTypes.TRANS_EXPENSES_TEMPLATE_FETCH_SUCCESS:
      return action.templates.reduce(
        (acc, v) => ({ ...acc, [v.templateId]: v }),
        {}
      );
    default:
      return state;
