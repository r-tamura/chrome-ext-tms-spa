...
const byId = (state: ById = {}, action: AttendanceAction) => {
  switch (action.type) {
    ...
    case ActionTypes.FETCH_FAILURE:
      return {
        ...state,
        [action.monthlyId]: monthly(state[action.monthlyId], action)
      };
    case ActionTypes.SUBMIT_APPLICATION_REQUEST:
    case ActionTypes.SUBMIT_APPLICATION_OK:
    case ActionTypes.SUBMIT_APPLICATION_NG:
      return {
        ...state,
        [action.payload.monthlyId]: monthly(
          state[action.payload.monthlyId],
          action
        )
      };
    case ActionTypes.FETCH_SUCCESS:
...
export type MonthlisReducer = ReturnType<typeof reducer>;
