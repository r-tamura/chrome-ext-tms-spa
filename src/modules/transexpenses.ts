...
interface ITransExpensesUpdateRequest extends Action {
  type:
    | ActionTypes.TRANS_EXPENSES_UPDATE_REQUEST
    | ActionTypes.TRANS_EXPENSES_DELETE_REQUEST;
  isUpdating: boolean;
}

interface ITransExpensesUpdateSuccess extends Action {
  type:
    | ActionTypes.TRANS_EXPENSES_UPDATE_SUCCESS
    | ActionTypes.TRANS_EXPENSES_DELETE_SUCCESS;
  result: ResultStatus;
  isUpdating: boolean;
}
...
interface ITransExpensesUpdateFailure extends Action {
  type:
    | ActionTypes.TRANS_EXPENSES_UPDATE_FAILURE
    | ActionTypes.TRANS_EXPENSES_DELETE_FAILURE;
  result?: ResultStatus;
  error?: object;
  isUpdating: boolean;
}
...
export const createExpense = (expense: TransExpense) => (
  ...
  try {
    composeAsync(successOrFailToUpdate(dispatch, getState), create)(expense);
  } catch (err) {
...
export const createExpenseFromTemplate = (templateId: string, date: string) => (
  ...
  const template = getTransExpenseTemplate(
...
export const deleteExpense = (expenseId: number) => (
  ...
  dispatch(updateTransExpensesRequest());
  try {
    composeAsync(successOrFailToUpdate(dispatch, getState), delete_)(expenseId);
  } catch (err) {
    dispatch(updateTransExpensesFailure({ error: err }));
  }
};

export const updateExpense = (expense: TransExpense) => (
  dispatch: Dispatch<AnyAction>,
  getState: () => RootState
) => {
  dispatch(updateTransExpensesRequest());
  try {
    composeAsync(successOrFailToUpdate(dispatch, getState), update)(expense);
  } catch (err) {
    dispatch(updateTransExpensesFailure({ error: err }));
  }
};
...
export type TransExpenseAction =
  | ITransExpensesFetchRequest
  | ITransExpensesFetchSuccess
  | ITransExpensesFetchFailure
  | ITransExpensesUpdateRequest
  | ITransExpensesUpdateSuccess
  | ITransExpensesUpdateFailure;
